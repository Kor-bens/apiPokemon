...
function displayPokemon() {
  ...
  pokedexContainer.innerHTML = "";
   ...
   buttonSearchButton.addEventListener("click", () => {
    ...
    fetch(`https://pokedex-pwu3.onrender.com/api/pokemons/${pokemonName}`, {
      method: "GET",
    })
      .then((response) => {
        if (!response.ok) {
    ...
    .then((dataPokemons) => {
      ...
      pokemons.forEach((pokemon) => {
        ...
        removeButton.addEventListener("click", () => {
          ...
          // Effectuer une requête DELETE à l'API pour supprimer le pokemon
          fetch(`https://pokedex-pwu3.onrender.com/api/pokemons/${pokemonId}`, {
            method: "DELETE",
          })
            .then((response) => {
              if (!response.ok) {
        ...
        pokedexContainer.appendChild(errorMessage);
          ...
          saveButton.addEventListener("click", () => {
            ...
            fetch(`https://pokedex-pwu3.onrender.com/api/pokemons/${pokemon.id}`, {
              method: "PUT",
              headers: {
